...
const _SPAWNS_PER_FRAME: int = 200
...
func set_height_params(step: float, exp: float) -> void:
	_height_step = step
	_height_exp = exp
...
func _upload_enemy(slot: int, _spawn_hex: Vector3i, world_pos: Vector3, hex_path: Array[Vector3i]) -> void:
	if not _gpu_initialized:
		return
	...
	_gpu_enemy_data[base + 3] = float(_all_path_cap)
	...
	var poff: int = _all_path_cap
	var plen: int = positions.size()
	var new_cap: int = poff + plen
	if new_cap * 3 > _gpu_path_data.size():
		_gpu_path_data.resize((new_cap + MAX_PATH_STEPS) * 3)
	_all_path_cap = new_cap
...
func _recompute_enemy_path(slot: int) -> void:
	...
	var hex_path := _compute_path(current_hex)
	if hex_path.is_empty():
		return
	var positions := _hex_path_to_positions(hex_path)
	if positions.size() < 2:
	...
	if plen > old_plen:
		...
		var new_cap: int = _all_path_cap + plen
		if new_cap * 3 > _gpu_path_data.size():
			_gpu_path_data.resize((new_cap + MAX_PATH_STEPS) * 3)
		_all_path_cap = new_cap
